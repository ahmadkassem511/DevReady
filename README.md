...
| `devready fix [path]` | **The "day two" doctor** — diagnoses and repairs a project that's set up but not working *right now*: a broken `.venv`/`node_modules`, a missing `.env`, a crashed container (shows its exit log before retrying), or a known error signature in the last run's log (missing module → reinstall; port conflict → names the process holding it, never auto-kills a stranger's process). Falls back to a read-only AI diagnosis when nothing deterministic applies — it explains, it never executes. Restarts the app so you see the result immediately. Also the **Fix** button in the GUI. |
